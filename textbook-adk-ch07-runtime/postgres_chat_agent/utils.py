...
async def call_agent_async(runner, user_input, session_id):
    ...
    try:
        session = await runner.session_service.get_session(
            app_name="postgres_chat_agent", user_id="demo_user", session_id=session_id
        )
    ...
    try:
        response = await runner.run_async(
            user_id="demo_user", session_id=session_id, content=content
        )
...
async def search_conversation_history(
    memory_service, query, app_name="postgres_chat_agent", user_id="demo_user"
):
    """
    ...
    try:
        ...
        return (
            f"🔍 Found {len(response.memories)} relevant conversations for '{query}':\n"
            + "\n".join(results)
        )
