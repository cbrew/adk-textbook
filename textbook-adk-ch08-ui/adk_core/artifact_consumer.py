...
class ConversationState:
    ...
    invocation_artifacts: dict[str, list[str]] = field(
...
class ArtifactEventConsumer:
    ...
    def extract_enhanced_event_info(self, event: dict) -> dict:
        ...
        return {
            ...
            "artifacts_in_invocation": list(
                self.state.invocation_artifacts.get(invocation_id, [])
            ),
            "all_artifacts": {
                name: {
                    "filename": info.filename,
                    "version": info.version,
                    "content_preview": info.content[:100] + "..."
                    if len(info.content) > 100
                    else info.content,
    ...
    def _update_artifact_tracking_from_responses(
        self, function_responses: list[dict] | None, invocation_id: str
    ):
                ...
                if filename:
                    if filename not in self.state.artifacts:
                        ...
                        self.state.artifacts[
                            filename
                        ].last_updated_in_invocation = invocation_id
    ...
    def _update_artifact_tracking_from_delta(
        self, artifact_delta: dict, invocation_id: str
    ):
...
async def demonstrate_improved_consumption():
    ...
    async with httpx.AsyncClient(timeout=None) as client:
        ...
        async for event_type, event_data in base_consumer.message(
            "make me a note about rhubarb and save it as an artifact"
        ):
            if event_type == "Event:" and isinstance(event_data, dict):
                enhanced_info = artifact_consumer.extract_enhanced_event_info(
                    event_data
                )
                ...
                if enhanced_info["function_calls"]:
                    for call in enhanced_info["function_calls"]:
                        ...
                        if "artifact_info" in call:
                            ...
                            print(
                                f"  -> Artifact: {artifact_info['filename']} ({artifact_info['content_length']} chars)"
                            )

                if enhanced_info["function_responses"]:
                    for response in enhanced_info["function_responses"]:
                        print(
                            f"Function Response: {response['name']} -> {response['response']}"
                        )
                ...
                if enhanced_info["artifacts_in_invocation"]:
                    print(
                        f"Artifacts in this invocation: {enhanced_info['artifacts_in_invocation']}"
                    )
