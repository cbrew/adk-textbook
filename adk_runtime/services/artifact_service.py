...
class PostgreSQLArtifactService(BaseArtifactService):
    ...
    def __init__(
        ...
        session_service: Optional["PostgreSQLSessionService"] = None,
    ):
        ...
        *,
        ...
        try:
            ...
            if file_size > self.MAX_ARTIFACT_SIZE:
                raise ValueError(
                    f"Artifact too large ({file_size} bytes, max {self.MAX_ARTIFACT_SIZE} bytes / 25MB). Consider splitting into smaller files."
                )
            ...
            artifact_event = Event(
                ...
                content=types.Content(
                    ...
                    parts=[
                        types.Part(
                            text=f"Created artifact '{filename}' (version {next_version}, {file_size} bytes)"
                        )
                    ],
                ),
                actions=EventActions(artifact_delta={filename: next_version}),
            )

            # Store in PostgreSQL BYTEA (pure BYTEA storage)
            storage_type = "postgresql"
            file_path = (
                f"pg://{app_name}/{user_id}/{session_id}/{filename}_v{next_version}"
            )
            file_data = content_bytes
            logger.debug(
                f"Using PostgreSQL BYTEA storage for {filename} ({file_size} bytes)"
            )

            # Save event to session first (to satisfy foreign key constraint)
            if self.session_service:
                # Save event and get the actual stored event ID
                stored_event_id = self.session_service.save_event(
                    session_id, artifact_event
                )
                logger.debug(
                    f"Saved artifact creation event {event_id} for session {session_id}"
                )
            else:
            ...
            logger.info(
                f"Saved artifact {filename} v{next_version} for session {session_id} (size: {file_size} bytes, storage: {storage_type})"
            )
            return next_version
    ...
    async def load_artifact(
        ...
        *,
        ...
        try:
            ...
            if not result:
                logger.debug(
                    f"Artifact {filename} v{version} not found in session {session_id}"
                )
                return None
            ...
            if metadata.get("app_name") != app_name:
                logger.warning(
                    f"Artifact {filename} app mismatch: expected {app_name}, got {metadata.get('app_name')}"
                )
                return None
            ...
            if file_data is None:
                logger.error(
                    f"Artifact {filename} has no file_data in PostgreSQL BYTEA storage"
                )
                return None
            ...
            logger.debug(
                f"Loaded {filename} from PostgreSQL BYTEA ({len(content_bytes)} bytes)"
            )
    ...
    async def list_artifact_keys(
        ...
        try:
            results = self.db.execute_query(
                QUERIES["get_session_artifacts"], (session_id,), fetch_all=True
            )
    ...
    async def delete_artifact(
        ...
        try:
            ...
            logger.info(
                f"Deleted {deleted_count} versions of artifact {filename} from session {session_id}"
            )
    ...
    def cleanup_orphaned_files(self) -> int:
        ...
        logger.info(
            "No filesystem cleanup needed - using pure PostgreSQL BYTEA storage"
        )
        return 0
