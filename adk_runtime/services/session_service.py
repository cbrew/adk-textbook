...
class PostgreSQLSessionService(BaseSessionService):
    ...
    async def create_session(
        ...
        *,
        ...
        session_data = {"app_name": app_name, **state}
    ...
    async def get_session(
        ...
        *,
        ...
        result = self.db.execute_query(
            QUERIES["get_session"], (session_id,), fetch_one=True
        )
        ...
        if stored_app_name != app_name or stored_user_id != user_id:
            logger.warning(
                f"Session {session_id} app/user mismatch: expected {app_name}/{user_id}, got {stored_app_name}/{stored_user_id}"
            )
            return None
        ...
        state = {k: v for k, v in session_data.items() if k not in ["app_name"]}
        ...
        return Session(
            ...
            state=state,
            events=events,
            last_update_time=result.get("updated_at", time.time()).timestamp()
            if hasattr(result.get("updated_at"), "timestamp")
            else time.time(),
        )
    ...
    async def delete_session(
        ...
        if not session:
            logger.warning(
                f"Cannot delete session {session_id}: not found or access denied"
            )
            return
        ...
        logger.info(
            f"Deleted session {session_id} for user {user_id} in app {app_name}"
        )

    def _load_session_events(
        self, session_id: str, config: Optional[GetSessionConfig] = None
    ) -> List[Event]:
        ...
        if config and config.num_recent_events:
            ...
            results = self.db.execute_query(
                QUERIES["get_session_events"], (session_id,), fetch_all=True
            )
    ...
    def update_session_state(
        self, session_id: str, state: Dict[str, Any], app_name: str, user_id: str
    ) -> None:
        """Update session state in the database."""
        try:
            # Prepare full state data including metadata
            full_state = {"app_name": app_name, **state}
