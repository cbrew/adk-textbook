...
class PostgreSQLMemoryService(BaseMemoryService):
    ...
    async def add_session_to_memory(self, session: "Session") -> None:
        ...
        try:
            ...
            logger.info(f"Indexed {len(session.events)} events for session {session.id}")
    ...
    async def _index_event_if_searchable(self, session: "Session", event) -> None:
        ...
        try:
            ...
            if self.embedding_service and content_summary:
                try:
                    embedding = await self.embedding_service.generate_embedding(content_summary)
                except Exception as e:
            ...
            logger.warning(f"Failed to index event {getattr(event, 'id', 'unknown')}: {e}")
    ...
    async def _search_by_text(
        ...
        try:
            ...
            index_results = self.db.execute_query(
                ...
                LIMIT %s
                """,
                (search_pattern, '%"is_index": true%', limit * 2),  # Get more indexes to find relevant events
                fetch_all=True
            )
            
            # Reconstruct memory entries from the referenced events
            index_results_list = index_results if isinstance(index_results, list) else []
            return await self._reconstruct_from_event_indexes(index_results_list, app_name, limit)
    ...
    async def _reconstruct_from_event_indexes(
        ...
        for index_result in index_results:
            try:
                ...
                memory_content = await self._create_memory_content_from_index(index_result, metadata)
                if memory_content:
    ...
    async def _create_memory_content_from_index(self, index_result: Dict, metadata: Dict) -> Optional[MemoryEntry]:
    ...
    def _extract_event_keywords(self, event) -> List[str]:
        ...
        try:
            ...
            if hasattr(event, 'content') and event.content and hasattr(event.content, 'parts'):
                for part in event.content.parts:
                    if hasattr(part, 'text') and part.text:
                        ...
                        filtered_words = [w for w in words if len(w) > 3 and w not in ['this', 'that', 'with', 'have', 'they', 'were', 'been', 'from']]
            ...
            if hasattr(event, 'actions') and event.actions and hasattr(event.actions, 'state_delta') and event.actions.state_delta:
                ...
                if "conversation_topic" in state_delta and state_delta["conversation_topic"]:
                    topic_words = str(state_delta["conversation_topic"]).lower().replace(',', ' ').split()
                ...
                searchable_state_fields = ['status', 'phase', 'current_task', 'focus_area']
    ...
    def _create_event_summary(self, event) -> str:
        ...
        try:
            ...
            if hasattr(event, 'get_function_calls'):
                function_calls = event.get_function_calls()
                if function_calls:
                    tool_names = [call.name for call in function_calls if hasattr(call, 'name')]
